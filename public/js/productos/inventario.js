    ...
    $.ajax(settings).done(function (response) {
       // console.log(response);
        ...
        $.each(response[0], function (key, val) {
            ...
            $("#contenido_inventario").append("<tr><td>" + nuevo_movimiento + "</td><td>" + val.SKU + "</td><td>" + val.NOM_PRODUCTO + "</td><td>" + val.CAN_INICIAL + "</td><td>" + val.CAN_ENTRADAS + "</td><td  style='display: none;'>" + val.CAN_SALIDAS + "</td><td>" + val.STOCK + "</td><td>" + val.PR_PRODUCTO + "</td><td>" + val.TOTAL + "</td><td style='display: none; '>" + val.COD_INVENTARIO + "</td></tr>");
        });
        ...
        var table = $('#table_inventario').DataTable({
            ...
            "buttons": [
                {
                    text: '<button  id="Mostrar_productos_I"   class="btn btn-primary"><i class="fa fa-arrow-left"></i>Productos</button>',
                ...
                {
                    text: '<button  id="mostrar_movimiento_productos"   class="btn btn-primary"><i class="fa fa-plus-circle"></i>Registrar Movimientos</button>',
                    action: function (e, dt, node, config) {
                    ...
                    customize: function(doc) {
                        doc.content.splice(1, 0, {
                          ...
                          columns: [{
                            ...
                            margin: [0, 5],
                            text: 'REPORTE DE INVENTARIO ***** GENERADO POR: ' + user_logeado,
                            alignment:'justify',
                            fontSize: 12
                          } 
                          ],  
...
function  mostrar_movimientos_inventario(){
    $('#table_inventario').dataTable().fnDestroy();
        var table = $('#table_inventario').DataTable({
            "bLengthChange": false,
            "bInfo": true,
...
function actualizar_inventario() {
    ...
    c_stock = $("#IAstock").val();
    nuevo_stock = (parseInt(c_entrada)+ parseInt(c_stock))
  //  nueva_entrada = (parseInt(c_entrada)+ parseInt(c_stock))
    ...
    var raw = JSON.stringify({
        "COD_INVENTARIO":cod_inventario.innerHTML,"ENTRADAS":c_entrada,"STOCK":nuevo_stock,"COD_MODULO":3,"COD_USUARIO":id_user
    });
    var requestOptions = {
        method: 'PUT',
        headers: myHeader,
...
function operacion() {
    ...
    var dd = document.getElementById('IAstock').value;
    if (cc == null || cc == "") cc = 0;
    if (dd == null || dd == "") dd = 0;
    var ee = document.getElementById('IAsalida').value;

    
    var rf = (parseInt(cc) + parseInt(dd));
