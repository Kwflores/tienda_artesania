...
function cargar_productos_sys() {
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response[0], function (key, val) {
            ...
            editar = "<div id='Mostrareditar_producto'><button type='input' id='editar" + val.COD_PRODUCTO + "' onclick='mostrar_actualizar_productos()' class='btn btn-round btn-lg btn-icon-only btn-secondary mx-2 mx-lg-3 mb-4'  data-toggle='tooltip' data-placement='left' title='Editar Producto'><i class='fas fa-pencil-alt' aria-hidden='true'></i></div>"
            eliminar = "<div id='MostrarEliminar_producto'><button type='input' id='refresh_pro" + val.COD_PRODUCTO + "' onclick='eliminar_producto(" + val.SKU + ")' class='btn btn-round btn-lg btn-icon-only btn-danger mx-2 mx-lg-3 mb-4'  data-toggle='tooltip' data-placement='left' title='Eliminar Producto'><i class='fa fa-trash' aria-hidden='true'></i></div>"
            img = "<img src='/public/img/categorias/" + val.URL_IMAGEN + "' height='100'  alt='" + val.NOM_PRODUCTO + "'>"
            $("#contenido_productos").append("<tr><td>" + editar + "</td><td>" + eliminar + "</td><td>" + img + "</td><td>" + val.SKU + "</td><td>" + val.NOM_PRODUCTO + "</td><td>" + val.DES_PRODUCTO + "</td><td>" + val.PR_PRODUCTO + "</td><td>" + val.NOM_CATEGORIA + "</td><td>" + val.NOM_PROVEEDOR + "</td><td>" + estado + "</td><td style='display: none; '>" + val.COD_PRODUCTO + "</td><td style='display: none; '>" + val.COD_PROVEEDORES + "</td><td style='display: none; '>" + val.COD_CATEGORIA + "</td><td style='display: none; '>" + val.COD_INVENTARIO + "</td><td style='display: none; '>" + val.URL_IMAGEN + "</td></tr>");
        });
        $('#table_productos').dataTable().fnDestroy();
        var table = $('#table_productos').DataTable({
            ...
            "buttons": [
                {
                    text: '<button  id="Mostrar_registro_P" class="btn btn-primary"><i class="fa fa-briefcase"></i> Nuevo Producto</button>',
                ...
                {
                    text: '<button id="Mostrar_productos_editar"  class="btn btn-secondary" ><i class="fas fa-pencil-alt"></i>Editar Productos</button>',
                    action: function (e, dt, node, config) {
                        permiso_editarProductos();
                    },
                    
                } ,
                ,
                {
                    text: '<button id="Mostrar_producto_eliminar"  class="btn btn-danger" ><i class="fa fa-trash"></i>Eliminar Productos</button>',
                    action: function (e, dt, node, config) {
                        permiso_eliminarProductos();
                    },
                    
                } ,
                {
                    text: '<button id="Mostrar_inventario_P" class="btn btn-secondary"><i class="fa fa-cube"></i> Inventario</button>',
                    action: function (e, dt, node, config) {
                        mostrar_inventario();
                    },
                    ...
                    customize: function (doc) {
                        doc.content.splice(1, 0, {
                            columns: [{
                                ...
                                margin: [0, 5],
                                text: 'REPORTE DE PRODUCTOS ***** GENERADO POR: ' + user_logeado,
                                alignment:'justify',
                                fontSize: 12
                            }]
                        });
        ...
        $('#table_productos tbody').on('click', 'tr', function () {
            ...
            actualizar_producto_estado(estado, data[10])
            $("#ASKU").val(data[3]);
            $("#Anom_producto").val(data[4]);
            $("#Adescripcion").val(data[5]);
            $("#Aprecio").val(data[6]);
            $("#Apcategoria").val(data[12]);
            $("#Approveedores").val(data[11]);
            $("#Am1").val(data[14]);
            document.getElementById("id_inventario").innerHTML = data[13]
            document.getElementById("id_producto").innerHTML = data[10]
...
function permiso_editarProductos(){
    $('#table_productos').dataTable().fnDestroy();
    $(document).ready(function () {
        var dt = $('#table_productos').dataTable({
            "bLengthChange": false,
            "bInfo": true,
            "pageLength": 10,
            "orderable": true,
...
function permiso_eliminarProductos(){
    $('#table_productos').dataTable().fnDestroy();
    $(document).ready(function () {
        var dt = $('#table_productos').dataTable({
            "bLengthChange": false,
